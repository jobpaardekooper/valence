...
impl LoadedChunk {
    ...
    fn calculate_sky_light_values(&self, world_surface: &[u32; 16 * 16]) -> Vec<u8> {
        let mut light = vec![0u8; self.height() as usize * 16 * 16];
    ...
    fn calculate_sky_light_section(
        ...
        let mut first_value = 0u8;
        let mut has_first_value = false;
        let mut is_uniform = true;
        let mut block_idx = 0usize;

        for local_y in 0u32..16 {
            for z in 0u32..16 {
                for x in 0u32..16 {
    ...
    pub(crate) fn write_init_packets(
        ...
        if init_packets.is_empty() {
            ...
            // due to OOM. So calculating the sky light sections from scratch here, meaning we don't have to store sky light
            // for most sections since most sky light sections in a chunk are NotSet (fully lit or fully dark).

            // This currently sky lighting implementation was based off the sky light description on the wiki:
            ...
            //         - Our current model treats them as either fully blocking or fully non-blocking, which is an approximation.
            //
            // 2. We don't calculate sky light across chunk boundaries.
            //     This is because we don't have the context here for other chunks. We approximate what how
            //     the sky light would propagate within the chunk itself, but we don't consider how neighboring
            //     chunks might affect the sky light at the edges of the chunk.
            //
            //     Vanilla handles this by running a chunk-light graph update across chunk boundaries:
            //         - It stores light in a global light engine (not isolated per chunk packet build).
            //         - When chunks load/change, it enqueues light updates.
            //         - Propagation crosses chunk edges into loaded neighbors.
            //         - If a neighbor isn’t loaded yet, updates are deferred/continued when that chunk becomes available.
            //         - Network packets then send changed light sections to clients incrementally (LightUpdate), instead of recomputing a chunk in isolation each time.
